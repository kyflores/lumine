...
def test(opt):
    ...
    untuned_result = tvmc.run(untuned_package, device='vulkan', benchmark=True, repeat=50)
    print(untuned_result.format_times())

    context = ["relay.backend.use_auto_scheduler=1"] if (opt.tune_type == "autoscheduler") else []
    print("Compiling tuned model")
    package = tvmc.compile(
        model,
        target=target,
        tuning_records=tune,
        pass_context_configs=context
    )
